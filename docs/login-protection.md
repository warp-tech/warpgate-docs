...
* **Max failures before IP block** - failed attempts allowed before the IP is blocked.
* **Failure window** - the period over which attempts are counted.
* **Initial block** - duration of the first block.
* **Backoff multiplier** - each repeated block is multiplied by this factor (e.g. `2.0` → 30min, 1h, 2h, …).
* **Max block** - the cap on a single block's duration.
...
* **Max failures before lockout** - failed attempts allowed before the account is locked.
* **Failure window** - the period over which attempts are counted.
* **Auto-unlock after timeout** - if enabled, locked accounts are released after the auto-unlock delay; otherwise they stay locked until an admin unlocks them.
* **Auto-unlock delay** - how long an account stays locked when auto-unlock is enabled.
* **Exempt admins from lockout** - users with an admin role are never locked out (enabled by default).
...
Blocked IPs and lockout records are retained for a configurable period (**Keep records for**) for auditing, after which they are pruned.
