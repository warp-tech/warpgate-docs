...
!!! note
    Teleport is an actively developed product and its capabilities and edition boundaries change over time. Treat the details below as a good-faith orientation, **last verified July 2026**, and check the [Teleport documentation](https://goteleport.com/docs/) for its current feature set.
...
| **Session recording** | Every protocol (see [below](#session-recording)) | SSH, Kubernetes and Windows desktop; databases/apps as audit events (see [below](#session-recording)) |
