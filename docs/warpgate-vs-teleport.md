...
| **Authentication** | Password, SSH public key, OTP (TOTP, with optional enforced enrollment), **SSO via OIDC** (Google, Microsoft, Apple or any custom provider; Kubernetes targets can use it via the `kubelogin` plugin), client certificates, in-browser approval, LDAP key sync | Short-lived certificates, MFA (TOTP / WebAuthn), SSO (GitHub in the open-source edition; **SAML/OIDC require Enterprise**) |
| **Self-service / JIT access** | Tickets, self-service **ticket requests** and per-connection **administrator approval** — included | Access Requests with approval workflows — Enterprise |
...
In RDP and VNC sessions Warpgate carries the text clipboard both ways, and both native RDP clients and the web desktop can resize the session live; clipboard file transfer and gateway-enforced clipboard policy are not implemented.
...
* **Session approvals** — mark a target as requiring administrator approval, and every connection to it is held after authentication until an admin lets it through from the admin UI, with approvals optionally remembered per user, target and source IP.
...
* You value a **small, auditable, Apache-2.0 codebase** where every feature — SSO, MFA enforcement, session recording, ticket requests, just-in-time approvals, clustering, credential encryption at rest — is in the free build.
